...
eRt = YPRToRot(yaw, pitch, roll); % rotation matrix of tool
ert = [0.2; 0 ; 0]; % position vector of tool

eTt = [eRt ert; 0 0 0 1]; % tool transformation
...
for i = t
    ...
    % Now update the kinematic model
    km.updateJacobian();  

    % Compute Cartesian reference velocities (x_dot)
    [x_dot, e_linear, e_angular]= cc.getCartesianReference(bTg); 

    % Debugging: Print the linear and angular errors
    disp('Linear Error:');
    disp(e_linear)
    disp('Angular Error:');
    disp(e_angular)
